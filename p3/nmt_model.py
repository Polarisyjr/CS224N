...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        ##不仅接收当前目标语言词的嵌入向量（维度为 embed_size），还需要接收上一时间步的上下文向量（通常是来自注意力机制，维度为 hidden_size）。
        self.h_projection = nn.Linear(hidden_size * 2, hidden_size, bias=False)  # Project concatenated hidden states to a new hidden state
        self.c_projection = nn.Linear(hidden_size * 2, hidden_size, bias=False)  # Project concatenated cell states to a new cell state
        self.att_projection = nn.Linear(hidden_size * 2, hidden_size, bias=False)  # Project encoder hidden states to a new hidden state
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X = X.permute(2, 0, 1)  # Revert to (src_len, b, e) for LSTM
        ##3. Compute `enc_hiddens`, `last_hidden`, `last_cell` by applying the encoder to `X`.
