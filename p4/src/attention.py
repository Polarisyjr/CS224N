...
def precompute_rotary_emb(dim, max_positions):
    ...
    # TODO: [part g]
    ### YOUR CODE HERE ###
    # 2(i - 1) for i in [1, dim/2] 即为从0到dim-2，步长为2，一个位置有dim/2个角度
    indices = torch.arange(0, dim, 2).float()
    inv_freq = 1.0 / (10000 ** (-indices/ dim))
    positions = torch.arange(0, max_positions).float() ##共max_positions个位置
    sinusoid_inp = torch.einsum('i,j->ij', positions, inv_freq)
    rope_cache = torch.cat((torch.cos(sinusoid_inp)[..., None], torch.sin(sinusoid_inp)[..., None]), dim=-1)
    ### END YOUR CODE ###
    return rope_cache
...
def apply_rotary_emb(x, rope_cache):
    ...
    # 确保 rope_cache 的形状匹配 x 的形状
    rope_cache = rope_cache[:, :, :seq_length, :]

    # 提取正弦和余弦值
    cos_cache = rope_cache[..., 0]
    sin_cache = rope_cache[..., 1]
    complex_cache = torch.view_as_complex(torch.stack((cos_cache, sin_cache), dim=-1))
    ...
    # 应用 RoPE
    x_rotated_complex = x_complex * complex_cache
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.rope:
            ...
            rope_cache = precompute_rotary_emb(dim, max_positions)
            ### END YOUR CODE ###
    ...
    def forward(self, x):
        ...
        if self.rope:
            ...
            ### YOUR CODE HERE ###
            rope_cache = self.rope_cache[:T, :].unsqueeze(0).unsqueeze(0)
