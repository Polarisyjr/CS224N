...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ### YOUR CODE HERE ###
        line = self.data[idx] # get single element

        #padding
        if len(line) < self.block_size:
            line += self.PAD_CHAR * (self.block_size - len(line))  # Pad the line if it's shorter than block_size
        elif len(line) > self.block_size:
            line = line[:self.block_size]
        # get span
        start = random.randint(0, len(line) - 1)
        span_length = random.randint(1, min(10, len(line) - start))  # Random span length up to 10 or till end of line
        end = start + span_length

        # Original text is used as the target
        target = [self.stoi[ch] for ch in line]

        # Corrupted text with masked span
        corrupted = list(line)
        corrupted[start:end] = [self.MASK_CHAR] * span_length
        corrupted = ''.join(corrupted) ##将字符列表合并回字符串

        #if len(corrupted) > self.block_size:
        #    corrupted = corrupted[:self.block_size]
        #elif len(corrupted) < self.block_size:
        #    corrupted += self.PAD_CHAR * (self.block_size - len(corrupted))

        corrupted_input = [self.stoi[ch] for ch in corrupted]

        return torch.tensor(corrupted_input, dtype=torch.long), torch.tensor(target, dtype=torch.long)
        ### END YOUR CODE ###
...
if __name__ == '__main__':
    ...
    if args.dataset_type == 'namedata':
        ...
        corruption_dataset = CharCorruptionDataset(
            open('wiki.txt', encoding='utf-8').read(), 128)
        for _, example in zip(range(8), corruption_dataset):
            x, y = example
